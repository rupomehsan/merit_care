...
class TeacherSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        for ($i=23; $i <28 ; $i++) { 
            ...
            for ($att=0; $att < 11 ; $att++) { 
                $attendence = new Attendence();
                $attendence->table = 'employee';
                $attendence->type = 'teacher';
                $attendence->table_id = $user->id;
                $attendence->branch_id = $institute_teacher->branch[0]->id;
                $attendence->date = Carbon::now()->subMonth(1)->subDay($i);
                $attendence->start_time = Carbon::parse('10:00:00')->subMinutes($i)->toTimeString();
                $attendence->end_time = Carbon::parse('19:00:00')->subMinutes($i)->toTimeString();

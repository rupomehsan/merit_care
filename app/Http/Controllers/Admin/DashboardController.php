...
class DashboardController extends Controller
{
    ...
    public function get_dashboard_stats() {
        ...
        $data['total_employees'] = User::whereMonth('created_at', Carbon::now()->month)
        ->whereExists(
            function($query) {  
                $query->from('user_user_role')
                    ->whereIn('user_user_role.user_role_id', [7])
    ...
    public function get_dashboard_stats_by_month() {
        
        $data = [];
        $data['total_expense'] = BranchAccountLog::where('type', 'debit')->whereBetween('created_at', [request()->from_date, request()->to_date])->where('is_expense', 1)->sum('amount');
        $data['total_income'] = BranchAccountLog::where('type', 'credit')->whereBetween('created_at', [request()->from_date, request()->to_date])->where('is_income', 1)->sum('amount');
        $data['total_customers'] = Customers::whereBetween('created_at', [request()->from_date, request()->to_date])->count();
        $data['total_orders'] = ProductOrder::whereBetween('created_at', [request()->from_date, request()->to_date])->count();
        $data['total_batch'] = InstituteClassBatch::whereBetween('created_at', [request()->from_date, request()->to_date])->count();
        $data['total_products'] = Product::whereBetween('created_at', [request()->from_date, request()->to_date])->count();
        $data['total_students'] = InstituteStudent::whereBetween('created_at', [request()->from_date, request()->to_date])->count();
        $data['total_teachers'] = InstituteTeacher::whereBetween('created_at', [request()->from_date, request()->to_date])->count();
    ...
    public function branch_dashboard_stats_by_month() {
        // dd(request()->all());
        $data = [];
        $data['total_expense'] = BranchAccountLog::where('type', 'debit')->whereBetween('created_at', [request()->from_date, request()->to_date])->where('branch_id', $this->getBranchID())->where('is_expense', 1)->sum('amount');
        $data['total_income'] = BranchAccountLog::where('type', 'credit')->whereBetween('created_at', [request()->from_date, request()->to_date])->where('branch_id', $this->getBranchID())->where('is_income', 1)->sum('amount');
        $data['total_customers'] = Customers::where('branch_id', $this->getBranchID())->whereBetween('created_at', [request()->from_date, request()->to_date])->count();
        $data['total_orders'] = ProductOrder::where('user_id', auth()->user()->id)->whereBetween('created_at', [request()->from_date, request()->to_date])->count();
        $data['total_batch'] = InstituteClassBatch::where('branch_id', $this->getBranchID())->whereBetween('created_at', [request()->from_date, request()->to_date])->count();
        $data['total_products'] = Product::whereBetween('created_at', [request()->from_date, request()->to_date])->count();
        $data['total_students'] = InstituteStudent::whereBetween('created_at', [request()->from_date, request()->to_date])->whereExists(
            function($query) {  
                $query->from('institute_branch_institute_student')
                    ->where('institute_branch_institute_student.institute_branch_id', $this->getBranchID())
                    ->whereColumn('institute_branch_institute_student.institute_student_id', 'institute_students.id');
            })->count();
        $data['total_teachers'] = InstituteTeacher::whereBetween('created_at', [request()->from_date, request()->to_date])->whereExists(
            function($query) {  
                $query->from('institute_branch_institute_teacher')
                    ->where('institute_branch_institute_teacher.institute_branch_id', $this->getBranchID())
                    ->whereColumn('institute_branch_institute_teacher.institute_teacher_id', 'institute_teachers.id');

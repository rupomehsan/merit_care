...
class ApiLoginController extends Controller
{
    ...
    private function getBranchID() {
        ...
        if(count($branch_user->branch_user) > 0) {
    ...
    public function user_update(Request $request)
    {
        ...
        if ($request->hasFile('photo')) {
            ...
            $user->photo = $path;
            if($this->getBranchID()) {
                $branch = InstituteBranch::where('id', $this->getBranchID())->update([
                    "image" => $path
                ]);
            }
            $user->save();
        ...
        if(Auth::check()){
            ...
            $auth_information = User::where('id',Auth::user()->id)
                ->with([
                    ...
                    'permissions'=>function($q1){
                        return $q1->select([
                            'title',
                            'permission_serial'
                        ]);
                    },
                    'branch_admin' => function($branch) {
                        $branch->with(['branch_details']);
                    }, 
                    'user_notification' => function($q2)  {
                        return $q2->with(['notification' => function($q2) {
                            $q2->latest();
                        }]);
                    }
                ])

...
class ExpenseController extends Controller
{
    ...
    public function all()
    {
        ...
        $accounts = $query->with('category')->paginate($paginate);
        return response()->json(['expenses' => $accounts], 200);
    }

    public function monthlyExpenseSheet($month) {
        $paginate = (int) request()->paginate;
        $orderBy = request()->orderBy;
        $orderByType = request()->orderByType;
        ...
        $total_monthly_expense = $query->whereMonth('created_at', $month)->sum('amount');
        $accounts = $query->with(['category', 'branch'])->whereMonth('created_at', $month)->paginate($paginate);
        return response()->json(['total_monthly_expense' => $total_monthly_expense,'expenses' => $accounts], 200);
    }

    public function admin_expense() : object {
        $paginate = (int) request()->paginate;
        $orderBy = request()->orderBy;
        $orderByType = request()->orderByType;

        $query = BranchAccountLog::where('is_expense', 1)
        ->orderBy($orderBy, $orderByType);

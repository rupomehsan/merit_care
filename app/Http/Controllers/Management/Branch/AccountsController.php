...
class AccountsController extends Controller
{
    ...
    public function admin_accounts_json()
    {
        $data = BranchAccount::where(function ($q) {
            $q->where('branch_id', null)
                ->orWhere('branch_id', $this->getBranchID());
        })->where('is_cash', 1)->get();
    ...
    {
        // dd(request()->all());
        $data = BranchAccount::where('id', request()->id)->with(['account_details'])->first();
        if (!$data) {
            return response()->json([
                'err_message' => 'validation error',
        ...
        $account_details = AccountDetails::where('account_id', $data->id)->first();
        // dd($account_details);
        $account_details->account_id = $data->id;
        $account_details->branch_id = $this->getBranchID();
        $account_details->account_no = request()->account_no;

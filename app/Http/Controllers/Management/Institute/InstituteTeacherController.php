...
class InstituteTeacherController extends Controller
{
    ...
    public function all()
    {
        ...
        )->with(['user', 'user_salary'])->paginate($paginate);
    ...
    {
        $teachers = InstituteTeacher::where('status', 1)->with(['user'])->get();
        return response()->json($teachers);
    }

    public function branch_all_teachers()
    {
        $data = InstituteTeacher::with(['user' => function ($qUser) {
            $qUser->with('branch_user');
        }])
            ->whereHas('user.branch_user', function ($subQuery) {
                $subQuery->where('institute_branch_id', $this->getBranchID());
            })
            ->get();
    ...
    {
        $data = InstituteTeacher::where('id', $id)->with(['batch', 'user_salary', 'user' => function ($q) {
    ...
    public function store()
    {
        ...
        $data->mail_link = request()->mail_link;
    ...
    public function update()
    {
        ...
        $user_salary = UserSalary::where('user_id', $user->id)->first();
        $user_salary->user_id = $user->id;
        $user_salary->branch_id = $this->getBranchID();

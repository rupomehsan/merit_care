...
class InstituteClassBatchTimeScheduleController extends Controller
{
    ...
    public function admin_schedules(): object
    {
        ...
        if (request()->has('branch_id') && request()->branch_id != 'undefined') {

            $query = $query->where('branch_id', request()->branch_id);
        }

        if (request()->has('teacher_id') && request()->teacher_id != 'undefined') {
            $query = $query->where('institute_class_teacher_id', request()->teacher_id);
        }
    ...
    public function all_routines()
    {
        ...
        foreach ($all_class as $key => $class_item) {
            $temp_array = [];
            $temp_array = $class_item;
            $class_batch = InstituteClassBatch::where('institute_class_id', $class_item->id)
                ->with(['subjects'])
                ->get();
        ...
        foreach ($scheduleArray as $key => $class) {
            if ($class["class_rowspan"]) {
                $row_count = 0;
                foreach ($scheduleArray as $t_class) {
                    if ($t_class["class_name"] == $class["class_name"] && $t_class["batch_name"] == $class["batch_name"]) {
                        $row_count++;
                    }
                }
                $scheduleArray[$key]["class_rowspan"] = $row_count;
    ...
    public function get_day_schedule($day_schedules, $day_name)
    {
        ...
        $temp_array["$day_name" . '_teacher'] = "";
        $temp_array["$day_name" . '_teacher_branch'] = "";
        ...
        foreach ($day_schedules as $day_schedule) {
            if (isset($day_schedule->day) && $day_schedule->day == $day_name) {
                ...
                $temp_array["$day_name" . '_teacher'] = $day_schedule->institute_class_teacher_id;
                $temp_array["$day_name" . '_null'] = false;

                if($day_schedule->teacher && $day_schedule->teacher->info){
                    $temp_array["$day_name" . '_teacher'] = $day_schedule->teacher->info->first_name.' '.$day_schedule->teacher->info->last_name;
                    $temp_array["$day_name" . '_teacher_branch'] = $day_schedule->teacher->info->branch_code;
                }
            }
        }
    ...
    public function store()
    {
        ...
        $institute_class_id = request()->institute_class_id;
        $institute_class_batch_id = request()->institute_class_batch_id;
        $schedules = request()->schedule;

        foreach ($schedules as $days) {

            foreach ($days as $day_data) {
                $day_data = (object) $day_data;
                $data = InstituteClassBatchTimeSchedule::where('start_time', $day_data->start_time)
                    ->where('end_time', $day_data->end_time)
                    ->where('room', $day_data->room_no)
                    ->where('day', $day_data->day)
                    ->where('institute_class_teacher_id', $day_data->teacher_id)
                    ->first();

                if ($data && $data->institute_class_id != $institute_class_id) {
                ...
                $data->start_time = $day_data->start_time;
                $data->end_time = $day_data->end_time;
                $data->room = $day_data->room_no;
                $data->branch_id = $this->getBranchID();
                $data->institute_class_id = $institute_class_id;
                $data->institute_class_batch_id = $institute_class_batch_id;
                $data->institute_class_subject_id = $day_data->institute_class_subject_id;
                $data->institute_class_teacher_id = $day_data->teacher_id;

                if($data->start_time && $data->end_time && $data->institute_class_teacher_id){
                    $data->save();
                }
            }
        }
    ...
    public function update()
    {
        ...
        $validator = Validator::make(request()->all(), [
            ...
            'institute_class_batch_id' => ['required'],
            'institute_class_subject_id' => ['required'],
            'institute_class_teacher_id' => ['required'],
        ]);

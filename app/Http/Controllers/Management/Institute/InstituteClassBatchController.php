...
class InstituteClassBatchController extends Controller
{
    private function getBranchID()
    {
        ...
        if (count($branch_admin->branch_admin) > 0) {
            ...
            return $branch_details->id ?? 0;
        } else {
            $branch_user = auth()->user()->load('branch_user');
            $branch_id = $branch_user->branch_user[0]->id ?? 0;
            return $branch_id;
        }
    }
    public function all()
    ...
    {
        $query = InstituteClassBatch::where('branch_id', $this->getBranchID())->orderBy('name', 'ASC');

        $InstituteClasss = $query->with(['class'])->get();
        return response()->json($InstituteClasss);
    }
    ...
    {
        $query = InstituteClassBatch::orderBy('name', 'ASC');

        if (request()->has('branch_id')) {
            $query = $query->where('branch_id', request()->branch_id);
        }

        $InstituteClassBatch = $query->with(['class', 'branch'])->get;
        return response()->json($InstituteClassBatch);
    }
    ...
    {
        $subjects = InstituteClassBatch::where('id', $id)->with([
            'subjects',
            'class'
        ])
            ->first();
    ...
    public function batch_related_subjects()
    {
        $subjects = InstituteClassBatch::where('id', request()->batch_id)
            ->with(['subjects', 'class'])->first();

...
const getters = {
    ...
    get_all_teachers: state => state.all_teachers,
};
...
const actions = {
    ...
    fetch_all_teachers: async function({ state, commit }) {
        let url = `/${api_prefix}/all-teachers`
        await axios.get(url)
        .then((res) => {
            this.commit('set_all_teachers', res.data);
        })
        .catch((e) => {
...
const mutations = {
    ...
    set_all_teachers: function(state, data) {
        state.all_teachers = data;
    },
};

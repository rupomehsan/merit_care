...
const actions = {
    ...
    [`fetch_${store_prefix}s`]: async function ({ state }) {
        ...
        await axios.get(url).then((res) => {
            this.commit(`set_${store_prefix}s`, res.data.incomes);
        });
    },
    /** override store */
    ...
    fetch_incomes_by_month: async function ({state}, event) {
        ...
        let url = `/${api_prefix}/fetch-income-by-month/${event.target.value}?page=${state[`${store_prefix}_page`]}&status=${state[`${store_prefix}_show_active_data`]}&paginate=${state[`${store_prefix}_paginate`]}`;

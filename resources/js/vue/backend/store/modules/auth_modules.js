...
const getters = {
    ...
    get_all_site_settings: state => state.site_settings,
}
...
const actions = {
    ...
    fetch_all_site_settings: function(state) {
        axios.get('/settings/all_site_settings')
            .then((res) => {
                $('#app_pre_loader').toggleClass('d-none');
                this.commit('set_site_settings', res.data);
            })
            .catch((err)=>{
                // this.commit('set_check_auth', false);
                // window.localStorage.removeItem('token');
...
const mutations = {
    ...
    set_site_settings: function (state, data) {

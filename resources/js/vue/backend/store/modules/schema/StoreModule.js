    ...
    */
    states(store_prefix = this.store_prefix, start_page = this.start_page, paginate = this.paginate) {
        ...
        const state = {
            ...
            [`${store_prefix}_from_date`]: ``,
            [`${store_prefix}_to_date`]: ``,
            [`orderByCol`]: "id",
            [`orderByAsc`]: false,
    ...
    actions() {
        ...
        let actions = {
            ...
            [`fetch_${store_prefix}s`]: async function ({ state }) {
                let end_point = location.origin + `/api/v1/${api_prefix}/all`;
                let url = new URL(end_point);

                url.searchParams.set('page', state[`${store_prefix}_page`]);
                url.searchParams.set('status', state[`${store_prefix}_show_active_data`]);
                url.searchParams.set('status', state[`${store_prefix}_show_active_data`]);
                url.searchParams.set('paginate', state[`${store_prefix}_paginate`]);
                url.searchParams.set('orderBy', state.orderByCol);

                if (state.orderByAsc) {
                    url.searchParams.set('orderByType', 'ASC');
                } else {
                    url.searchParams.set('orderByType', 'DESC');
                }

                if (state[`${store_prefix}_search_key`]) {
                    url.searchParams.set('search_key', state[`${store_prefix}_search_key`]);
                }
                if (state[`${store_prefix}_from_date`] && state[`${store_prefix}_to_date`]) {
                    url.searchParams.set('from_date', state[`${store_prefix}_from_date`]);
                    url.searchParams.set('to_date', state[`${store_prefix}_to_date`]);
                }
            ...
            [`upload_${store_prefix}_create_canvas_input`]: function ({
                ...
                axios
                    ...
                    .then((res) => {
                        window.s_alert(`new ${store_prefix} is created`);
                        this.commit(
    ...
    mutations() {
        ...
        const mutations = {
            ...
            * */
            ...
            [`set_${store_prefix}_from_date`]: debounce(function (state, search_key) {
                state[`${store_prefix}_from_date`] = search_key;
            }, 100),
            [`set_${store_prefix}_to_date`]: debounce(function (state, search_key) {
                state[`${store_prefix}_to_date`] = search_key;
            }, 100),
            ...
            /** set selected push only data array */

            [`set_selected_${store_prefix}s_push`]: function (state, data) {
                let temp_selected = state[`${store_prefix}_selected`];

...
const actions = {
    ...
    [`fetch_${store_prefix}s`]: async function ({ state }, data) {
        ...
        if(data && data.teacher_id != '') {
            url = `/${api_prefix}/all?page=${state[`${store_prefix}_page`]}&status=${state[`${store_prefix}_show_active_data`]}&paginate=${state[`${store_prefix}_paginate`]}&teacher_id=${data.teacher_id}`;
        }
        if(data && data.teacher_id != '' && data.branch_id != '') {
            url = `/${api_prefix}/all?page=${state[`${store_prefix}_page`]}&status=${state[`${store_prefix}_show_active_data`]}&paginate=${state[`${store_prefix}_paginate`]}&teacher_id=${data.teacher_id}&branch_id=${data.branch_id}`;
        }

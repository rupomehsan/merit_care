        ...
        <div class="card list_card">
                ...
                <table class="table table-hover table-striped table-bordered">
                    <thead class="table-light">
                        <tr>
                            ...
                            <table-th :sort="false" :tkey="'category_id'" :title="'Category'" />
                            <table-th :sort="true" :tkey="'salary_amount'" :title="'Salary amount'" />
                            <table-th :sort="true" :tkey="'date'" :title="'date'" />
                            <th aria-label="actions">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="table-border-bottom-0">
                        <tr v-for="item in this[`get_teacher_salary_statements`].data" :key="item.id">
                            <td>
                            ...
                            <td>
                                <span class="text-primary" v-if="item.category">
                                    {{ item.category.title }}
                                </span>
                            </td>
                            ...
                            <td>
                                <div class="table_actions">
                                    ...
                                    <ul>
                                        <li>
                                            <a href="javascript:void(0)" @click.prevent="call_store(`set_${store_prefix}`,item)">
            ...
            <div class="card-footer py-1 border-top-0">
                <div class="d-inline-block">
                    <pagination :data="this[`get_teacher_salary_statements`]" :limit="5" :size="'small'" :show-disabled="true" :align="'left'"
                ...
                <div class="show-limit d-inline-block">
                    <span>Total:</span>
                    <span>{{ this[`get_teacher_salary_statements`].total }}</span>
                </div>
            </div>
        </div>
...
export default {
    ...
    created: function(){
        this[`fetch_teacher_salary_statements`]();
    },
    methods: {
        ...mapActions([
            `fetch_teacher_salary_statements`,
        ...
        check_if_data_is_selected: function(user){
            let check_index = this[`get_${store_prefix}_selected`].findIndex((i) => i.id == id);
            if(check_index >= 0){
                return true;
            }else{
                return false;

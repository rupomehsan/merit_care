                        ...
                        <div class="col-lg-11">
                            ...
                            <div class="mb-3" data-repeater-list="group-a">
                                <div class="repeater-wrapper pt-0 pt-md-4" data-repeater-item>
                                    ...
                                    <div class="border border-info p-2 mb-2 rounded-sm" v-for="(subject, index) in subjects" :key="index">
                                        <h4>{{ subject.title }}</h4>
                                        <div class="table-responsive">
                                            <table class="table text-nowrap">
                                                ...
                                                <tbody>
                                                    <tr v-for="day in days" :key="day+index" class="schedule_days">
                                                        <td>
                                                            <input type="hidden" :value="subject.id" :name="`schedule[${index}][${day}][institute_class_subject_id]`">
                                                            <input type="hidden" :value="day" :name="`schedule[${index}][${day}][day]`">
                                                            {{ day }}
                                                        </td>
                                                        <td>
                                                            <input type="time"
                                                                :id="`${day}_${subject.id}_start_time`"
                                                                :name="`schedule[${index}][${day}][start_time]`"
                                                                class="form-control invoice-item-price "
                                                                placeholder="Start time" />
                                                        </td>
                                                        <td>
                                                            <input type="time"
                                                                :id="`${day}_${subject.id}_end_time`"
                                                                :name="`schedule[${index}][${day}][end_time]`"
                                                                class="form-control invoice-item-price "
                                                                placeholder="End time" />
                                                        </td>
                                                        <td>
                                                            <input type="number"
                                                                :id="`${day}_${subject.id}_room_no`"
                                                                :name="`schedule[${index}][${day}][room_no]`"
                                                                class="form-control invoice-item-price "
                                                                placeholder="Room no" />
                                                        </td>
                                                        <td style="min-width: 200px;">
                                                            <select
                                                                :id="`${day}_${subject.id}_teacher_id`"
                                                                :name="`schedule[${index}][${day}][teacher_id]`"
                                                                class="form-select">
...
export default {
    ...
    methods: {
        ...
        set_values: function(){
            let that = this;
            setTimeout(() => {
                console.log(that.get_batch_subjects);
                that.get_batch_subjects.subjects.forEach(subject=>{
                    subject.time_schedule.forEach(time=>{
                        let selector = `${time.day}_${subject.id}_start_time`;
                        let el = document.getElementById(selector);
                        // console.log(el,selector);
                        if(el){
                            el.value = time.start_time;
                        }

                        selector = `${time.day}_${subject.id}_end_time`;
                        el = document.getElementById(selector);
                        if(el){
                            el.value = time.end_time;
                        }

                        selector = `${time.day}_${subject.id}_room_no`;
                        el = document.getElementById(selector);
                        if(el){
                            el.value = time.room_no;
                        }
                        ...
                        selector = `${time.day}_${subject.id}_teacher_id`;
                        el = document.getElementById(selector);
                        if(el){
                            el.value = time.institute_class_teacher_id;
                        }
                    })
                })
            }, 2000);

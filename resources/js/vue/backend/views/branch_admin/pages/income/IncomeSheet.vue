            ...
            <div class="table-responsive card-body text-nowrap">
                <div class="row">
                    <div class="col-md-6 mb-2">
                        ...
                        <select v-model="month" @change="filter_incomes_by_month($event)" class="form-select" name="month" id="month">
                ...
                <table class="table table-hover table-striped table-bordered">
                    <thead class="table-light">
                        <tr>
                            ...
                            <table-th :sort="true" :tkey="'category'" :title="'Category'" />
                            <table-th :sort="true" :tkey="'total_amount'" :title="'Total Amount'" />
                    ...
                    <tbody class="table-border-bottom-0">
                        <tr v-for="item in this[`get_${store_prefix}s`].data" :key="item.id">
                            ...
                            <td>{{ item.id }}</td>
                            <td><span class="text-primary">{{ item.date }}</span></td>
                            <td>
                        ...
                        <tr>
                            <td colspan="4"><span class="fw-bold">Total</span></td>
                            <td>{{ this[`get_${store_prefix}s`].total_monthly_income ? this[`get_${store_prefix}s`].total_monthly_income : subTotal }}</td>
                            <!-- <td></td> -->
                        </tr>
                    </tbody>
                </table>
            ...
            <div class="card-footer py-1 border-top-0">
                ...
                <div class="show-limit d-inline-block">
                    ...
                    <select @change.prevent="call_store(`set_${store_prefix}_paginate`,$event.target.value)">
                        <option v-for="i in [10,5,25,50,100]" :key="i" :value="i" :selected="i === 50">
                            {{ i }}
                        </option>
                    </select>
                </div>
...
export default {
    ...
    methods: {
        ...
        filter_incomes_by_month: function(event) {
            this.fetch_incomes_by_month(event);
        },
        ...mapMutations([
    ...
    computed: {
        ...
        subTotal: function() {
            var total_amount = 0;
            // console.log(this.get_incomes);
            if(this.get_incomes.data && this.get_incomes.data.length > 0) {
                this.get_incomes.data.forEach(element => {

                                ...
                                <div class="form-group d-grid align-content-start gap-1 mb-2 " >
                                    <div>
                                        ...
                                        <select name="subject_id" id="defaultSelect" class="form-select">
                                            ...
                                            <option
                                            v-for="(item, index) in get_batch_releated_subjects"
                                            :key="index"
                                            :value="item.id"
                                            >{{ item.title }}</option>
                                        </select>
...
export default {
    ...
    watch: {
        get_batch_selected: function(v){
            console.log(v);
            if(v.length){
                this.fetch_class_batch_subject_for_exam({
                    class_id: v[0].institute_class_id,
                    branch_id: v[0].branch_id,
                    batch_id: v[0].id,
                });
            }
        },

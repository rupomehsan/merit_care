                ...
                <table class="table table-hover table-striped table-bordered">
                    <thead class="table-light">
                        <tr>
                            ...
                            <table-th :sort="true" :tkey="'title'" :title="'Name'" />
                            <table-th :sort="true" :tkey="'is_visible'" :title="'Visibility'" />
                        ...
                        <tr v-for="item in this[`get_${store_prefix}s`].data" :key="item.id">
                            ...
                            <td>
                                <div class="form-check form-switch d-flex justify-content-center">
                                    <input class="form-check-input" v-if="item.is_visible == 1" checked @change="process_visibility(item.id, $event)" type="checkbox" role="switch" :id="`SwitchPresent${item.id}`">
                                    <input class="form-check-input" v-else @change="process_visibility(item.id, $event)" type="checkbox" role="switch" :id="`SwitchVisible${item.id}`">
                                </div>
                            </td>
                            <td>
                                <div class="table_actions">
...
export default {
    ...
    methods: {
        ...
        process_visibility(account_id, event) {
            
            let data = {
                account_id: account_id,
                visibility: event.target.checked
            }
            this.category_visibility_update(data)

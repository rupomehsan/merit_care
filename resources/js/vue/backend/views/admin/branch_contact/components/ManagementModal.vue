                        ...
                        <tbody class="table-border-bottom-0">
                            <tr v-for="item in this[`get_${store_prefix}s`].data" :key="item.id">
                                <td>
                                    ...
                                    <input v-else @change="call_store(`set_branch_for_batch`,item)" type="checkbox" class="form-check-input">
                                </td>
                                <td>{{ item.id }}</td>
                                <td>
...
export default {
    ...
    methods: {
        ...
        check_if_data_is_selected: function(item){
            let check_index = this[`get_${store_prefix}_selected`].findIndex((i) => i.id == item.id);
            if(check_index >= 0){
                return true;
            }else{
                return false;

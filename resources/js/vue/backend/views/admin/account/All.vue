                ...
                <table class="table table-hover table-striped table-bordered">
                    <thead class="table-light">
                        <tr>
                            ...
                            <table-th :sort="false" :tkey="'branch'" :title="'Branch'" />
                            <table-th :sort="false" :tkey="'account_no'" :title="'Account number'" />
                            <table-th :sort="false" :tkey="'account_mobile_no'" :title="'Account Mobile Number'" />
                            <table-th :sort="false" :tkey="'total_expense'" :title="'Total Expense'" />
                            <table-th :sort="false" :tkey="'total_income'" :title="'Total Income'" />
                            <table-th :sort="false" :tkey="'net_balance'" :title="'Net Balance'" />
                    ...
                    <tbody class="table-border-bottom-0">
                        <tr v-for="item in this[`get_${store_prefix}s`].data" :key="item.id">
                            ...
                            <td>{{ item.id }}</td>
                            <td>
                                <span class="text-warning cursor_pointer text-capitalize" @click.prevent="call_store(`set_${store_prefix}`,item)">
                                    {{ item.title }}
                                </span>
                            </td>
                            <td>
                                <span class="text-primary text-capitalize" v-if="item.institute_branch">
                                    {{ item.institute_branch.name }}
                                </span>
                            </td>
                            <td v-if="item.account_details">{{ item.account_details.account_no }}</td>
                            <td v-if="item.account_details">{{ item.account_details.account_mobile_no }}</td>
                            <td>{{ item.expense_log_sum_amount }}</td>
                            <td>{{ item.income_log_sum_amount }}</td>
                            <td>{{ item.income_log_sum_amount - item.expense_log_sum_amount }}</td>
                            <td>
                                <div class="table_actions">

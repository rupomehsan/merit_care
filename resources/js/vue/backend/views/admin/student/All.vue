        ...
        <div class="card list_card">
            <div class="card-header">
                <h4>
                    ...
                    <small v-if="this[`get_${store_prefix}_selected`].length">
                        ...
                        &nbsp;
                        <b @click="call_store(`set_clear_selected_${store_prefix}s`, true)"
                            class="text-danger cursor-pointer">clear</b>
                        &nbsp;
                        <b @click="call_store(`set_${store_prefix}_show_selected`, true)"
                            class="text-success cursor-pointer">show</b>
                    </small>
                </h4>
                <div class="search">
                    <form action="#">
                        <input @keyup="call_store(`set_${store_prefix}_search_key`, $event.target.value)"
                            class="form-control border border-info" placeholder="search..." type="search">
                    </form>
                </div>
                <div class="btns d-flex gap-2 align-items-center">
                    <permission-button :permission="'can_create'" :to="{ name: `Create${route_prefix}` }"
                    ...
                    <div class="table_actions">
                        ...
                        <ul>
                            ...
                            <li>
                                <a href="#" v-if="this[`get_${store_prefix}_show_active_data`]"
                                    title="display data that has been deactivated"
                                    @click.prevent="call_store(`set_${store_prefix}_show_active_data`, 0)"
                                    class="d-flex">
                                    <i class="fa-regular fa-hand-point-right"></i>
                                    Deactivated data
                                </a>
                                <a href="#" v-else title="display data that are active"
                                    @click.prevent="call_store(`set_${store_prefix}_show_active_data`, 1)"
                                    class="d-flex">
                                    <i class="fa-regular fa-hand-point-right"></i>
                                    Active data
                                </a>
            ...
            <div class="table-responsive card-body text-nowrap">
                <div class="row">
                    <div class="col-6">
                        <h5> Branch</h5>
                        <select name="branch_id" class="form-select mb-3" v-model="branch_id"
                            @change="call_student_by_branch($event)" id="branch_id">
                            <option value="">Select Branch</option>
                            <option v-for="(branch, index) in get_all_branches" :key="index" :value="branch.id">{{
                        branch.name }}</option>
                        </select>
                    </div>
                    <div class="col-6">
                        <h5> Role</h5>
                        <input type="text" @change="call_student_by_roll($event)" class="form-control">
                    </div>
                    <!-- <div class="col-6">
                        <h5> Gender</h5>
                        <select name="branch_id" class="form-select mb-3" @change="filter_student_by_gender($event)" id="branch_id">
                ...
                <table class="table table-hover table-striped table-bordered">
                    <thead class="table-light">
                        <tr>
                            <th><input @click="call_store(`set_select_all_${store_prefix}s`)" type="checkbox"
                                    class="form-check-input check_all"></th>
                            <table-th :sort="true" :tkey="'id'" :title="'ID'" :ariaLable="'id'" />
                            <table-th :sort="false" :title="'Roll'" :ariaLable="'user_id'" />
                            <table-th :sort="false" :title="'Branch'" :ariaLable="'branch'" />
                            <table-th :sort="true" :tkey="'name'" :title="'Name'" />
                            <table-th :sort="false" :tkey="'gender'" :title="'Gender'" />
                            <table-th :sort="true" :tkey="'email'" :title="'Email'" />
                            <table-th :sort="true" :tkey="'phone'" :title="'Phone'" />
                            <table-th :sort="true" :tkey="'class'" :title="'batch & class'" />
                    ...
                    <tbody class="table-border-bottom-0">
                        <tr v-for="item in this[`get_${store_prefix}s`].data" :key="item.id">
                            <td>
                                <input v-if="check_if_data_is_selected(item)" :data-id="item.id" checked
                                    @change="call_store(`set_selected_${store_prefix}s`, item)" type="checkbox"
                                    class="form-check-input">
                                <input v-else @change="call_store(`set_selected_${store_prefix}s`, item)"
                                    type="checkbox" class="form-check-input">
                            </td>
                            <td>{{ item.id }}</td>
                            <td>{{ item.user_id }}</td>
                            <td v-if="item.user"><span v-if="item.user.branch_user.length" class="text-primary">
                                    {{ item.user.branch_user[0].name }}</span></td>

                            <td v-if="item.user">
                                <span class="text-warning cursor_pointer"
                                    @click.prevent="call_store(`set_${store_prefix}`, item)">
                                    {{ item.user.first_name }} {{ item.user.last_name }}
                                </span>
                            </td>
                            <td><span class="text-info">{{ item.gender }}</span></td>
                            <td v-if="item.user">
                                <span>
                                    {{ item.user.email }}
                                </span>
                            ...
                            <td>
                                <ul class="list-group" v-for="(batch, index) in item.institute_class_batches"
                                    :key="index">
                                    <li class="list-group-item">{{ batch.name }} - {{ batch.class.title }}</li>
                                </ul>
                            </td>
                            ...
                            <td>
                                <div class="table_actions">
                                    ...
                                    <ul>
                                        ...
                                        <li>
                                            <permission-button :permission="'can_create'"
            ...
            <div class="card-footer py-1 border-top-0">
                <div class="d-inline-block">
                    <pagination :data="this[`get_${store_prefix}s`]" :limit="5" :size="'small'" :show-disabled="true"
                        :align="'left'" @pagination-change-page="handle_pagination">
...
export default {
    ...
    created: function () {
        ...
        this[`set_${store_prefix}_orderByCol`]('id');
    },
    methods: {
        ...mapActions([
        ...
        call_student_by_roll: function (event) {
        ...
        call_student_by_branch: function (event) {
            // console.log(event.target.value);
        ...
        filter_student_by_gender: function (event) {
            // console.log(event.target.value);

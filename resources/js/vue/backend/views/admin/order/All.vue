                ...
                <table class="table table-hover table-striped table-bordered">
                    <thead class="table-light">
                        <tr>
                            ...
                            <table-th :sort="true" :tkey="'invoice_id'" :title="'Invoice ID'" />
                            ...
                            <table-th :sort="true" :tkey="'paid_amount'" :title="'Paid Amount'" />
                    ...
                    <tbody class="table-border-bottom-0">
                        <tr v-for="item in this[`get_${store_prefix}s`].data" :key="item.id">
                            ...
                            <td class="text-info fw-bold">{{ item.total_amount }}</td>
                            <td>{{ item.paid_amount }}</td>
                            <td>
                                <span v-if="item.payment_status == 'accepted'" class="badge bg-success">{{ item.payment_status }}</span>
                                <span v-else class="badge bg-secondary">{{ item.payment_status }}</span>
                            </td>
                            <td>
                                <span v-if="item.status == 'accepted' || item.status == 'delivered'" class="badge bg-success">{{ item.status }}</span>
                                <span v-else-if="item.status == 'canceled'" class="badge bg-danger">{{ item.status }}</span>
                                <span v-else class="badge bg-primary">{{ item.status }}</span>
                            </td>
                            <td>
                                <div class="table_actions">
